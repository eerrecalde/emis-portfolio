...
import { skillAreas, type PortfolioSkill, type SkillArea } from '../types/portfolio';
...
export function SkillsPage({ skills }: SkillsPageProps) {
  ...
  const availableAreas = useMemo(
    () =>
      skillAreas.filter((area) => skills.some((skill) => skill.areas.includes(area))),
    [skills],
  );
  ...
  const chartHeight =
    TOP_PADDING + Math.max(visibleSkills.length, 1) * ROW_HEIGHT + BOTTOM_PADDING;
  ...
  useEffect(() => {
    const updateWidth = () => setChartWidth(chartRef.current?.clientWidth || 360);
    updateWidth();
  ...
  return (
    <>
      ...
      <figure className="mt-10" aria-labelledby="experience-arcs-title">
        <div className="flex flex-wrap items-baseline justify-between gap-4">
          <h2 className="text-xl font-semibold tracking-tight text-slate-50" id="experience-arcs-title">
            Experience arcs
          </h2>
        ...
        >
          ...
          <defs>
            <linearGradient id="experience-arc-gradient" x1="0" x2="1" y1="0" y2="0">
          ...
          {sortedSkills.map((skill) => {
            ...
            const y = TOP_PADDING + (visibleIndex.get(skill.id) ?? visibleSkills.length) * ROW_HEIGHT;
